...
export function MessageCapture({ visible, onClose, initialMode = 'task', initialTitle = '' }: { visible: boolean; onClose: () => void; initialMode?: 'task' | 'renewal'; initialTitle?: string }) {
  ...
  return (
    <AppBottomSheet accessibilityLabel="Close add a plan" onClose={close} title="Add a plan" visible={visible}>
      <View style={styles.row}>
        {(['task', 'renewal'] as const).map((value) => (
          <Pressable
            accessibilityRole="button"
            accessibilityState={{ selected: mode === value }}
            disabled={busy}
            key={value}
            onPress={() => { setMode(value); reset(); }}
            style={[styles.button, mode === value && styles.selected]}>
            <AppText>{value === 'task' ? 'Task' : 'Renewal'}</AppText>
          </Pressable>
        ))}
      </View>
      ...
      <DateTimeField
        ...
        disabled={busy}
        label={mode === 'renewal' ? 'Next renewal date (optional)' : 'Due date / time (optional)'}
        mode="datetime"
        onChange={(value) => { setDate(value); reset(); }}
        value={date}
      />
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: colors.neutral[400], borderRadius: borderRadius.md, padding: spacing.md, color: colors.neutral[900], backgroundColor: colors.neutral[0], minHeight: 48 },
  error: { color: colors.semantic.dangerDark },
  button: { flex: 1, minHeight: 48, justifyContent: 'center', alignItems: 'center', padding: spacing.md, borderRadius: borderRadius.md, backgroundColor: colors.neutral[200] },
  selected: { backgroundColor: colors.primary[100] },
  preview: { padding: spacing.lg, gap: spacing.sm, backgroundColor: colors.neutral[0], borderRadius: borderRadius.lg },
});
